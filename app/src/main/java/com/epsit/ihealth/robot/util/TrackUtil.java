    ...
    public static void drawAnim(List<YMFace> faces, SurfaceView outputView, float scale_bit, int cameraId, String fps, boolean showPoint) {
        ...
        try {
            ...
            for (int i = 0; i < faces.size(); i++) {
                ...
                int size = DisplayUtil.dip2px(RobotApplication.getInstance(), 2);
                ...
                int smailSize = DisplayUtil.dip2px(RobotApplication.getInstance().getApplicationContext(), 1.5f);
...
//                canvas.drawLine(x1, y1 + heng, x1, y1 - length, paint);
                if (showPoint) {
                    ...
                    size = DisplayUtil.dip2px(RobotApplication.getInstance(), 2.5f);
                    paint.setStrokeWidth(size);
